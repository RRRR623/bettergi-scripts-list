...
// 修复点：添加全局后台任务引用，便于主脚本等待其完成
let detectExpOrMoraTask = null;
...
(async function () {
    ...
    try {
        ...
        running = false;
        // 等待后台任务完全结束，避免 V8 释放资源后 .NET Task 继续访问导致 ObjectDisposedException
...
async function executeSingleFriendshipRound(roundIndex, ocrTimeout, fightTimeout, enemyType) {
    ...
    // 5) 成功则执行战斗后流程；失败则回退到准备/触���点并进入下一轮
    // 导航到触发点
...
async function AutoFriendshipDev(times, ocrTimeout, fightTimeout, enemyType = "盗宝团") {
    ...
    try {
        if (settings.loopTillNoExpOrMora) {
            // 修复点：将后台任务赋值给全局变量，便于主脚本 finally 块等待
        ...
        running = false;
        // 修复点：主脚本的 finally 已经处理等待，这里只作保险
        if (detectExpOrMoraTask) {
...
async function detectExpOrMora() {
    ...
    // 修复点：在循环开始和关键位置增加 running 检查，确保能及时响应脚本退出信号
    while (running) {
        // 修复点：循环开始处检查 running 标志
        if (!running) break;
        
        try { await sleep(1); } catch (e) { break; }
        
        // 修复点：检查后再执行 OCR，避免在资源已释放后仍访问
        if (!running) break;
        ...
        if (!detectedExpOrMora) {
            ...
            let res2 = null;
            try {
                // 修复点：获取资源前再次检查 running
                if (!running) break;
                
                gameRegion = captureGameRegion();
                
                // 修复点：获取资源后立即检查 running，防止资源释放时仍在使用
                if (!running) break;
